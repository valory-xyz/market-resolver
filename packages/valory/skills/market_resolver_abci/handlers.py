...
"""This module contains the handlers for the market resolver skill."""
...
from packages.valory.skills.market_resolver_abci.dialogues import (
    HttpDialogue,
    HttpDialogues,
)
...
class HttpHandler(BaseHttpHandler):
    ...
    def setup(self) -> None:
        ...
        # Route regexes
        hostname_regex = (
            rf".*({service_endpoint_base}|{propel_uri_base_hostname}|"
            rf"localhost|127.0.0.1|0.0.0.0)(:\d+)?"
        )
    ...
    def _get_handler(self, url: str, method: str) -> Tuple[Optional[Callable], Dict]:
        ...
        where hostname is allowed to be localhost, 127.0.0.1, 0.0.0.0,
        the service_endpoint_base hostname, or a Propel agent hostname.

        :param url: the url to check
        :param method: the http method
        :returns: the handling method if matched, otherwise None, and the
            captured kwargs from the regex.
        """
        # Check base url
        ...
        # No route found
        self.context.logger.info(
            f"The message [{method}] {url} is intended for the HttpHandler "
            f"but did not match any valid pattern"
        )
    ...
    def _handle_get_health(
        ...
        placeholders kept for shape parity with the mech service -- market-resolver
        ...
        # Readiness/progress: market-resolver has no backlog metric, so these
        # are always-ok placeholders kept for mech parity.
